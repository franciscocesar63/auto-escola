...
CONSTRAINT uk_aula UNIQUE KEY(codigo)   
)ENGINE=InnoDB;
...
CREATE FUNCTION gera_codigo_aula(param_id INT) RETURNS VARCHAR(10)	
	BEGIN
		IF param_id < 10 THEN
			RETURN concat('A000',param_id); 
        ELSEIF param_id >= 10 AND param_id <= 99 THEN
			RETURN concat(ano, 'A00' , param_id);
		ELSEIF param_id > 99 AND param_id <= 999 THEN
			RETURN concat(ano, 'A0' , param_id);
		ELSE RETURN concat(ano, 'A',param_id);
        END IF;
	END $$
...
CREATE FUNCTION retorna_id_codigo_aula(param_codigo VARCHAR(10)) RETURNS INT
	BEGIN
		RETURN (SELECT aula.id FROM aula WHERE aula.codigo = para_codigo);
    END $$
...
CREATE PROCEDURE stp_inserir_aula_teorica(param_quantidade ENUM('1', '2', '3'), param_id_instrutor INT, param_assunto VARCHAR(30))
	BEGIN
		SET TRANSACTION ISOLATION LEVEL READ COMMITTED;
		INSERT INTO aula(tipo, codigo, quantidade, id_instrutor) VALUES('teorica', gera_codigo_aula(last_insert_id()),param_quantidade, param_id_instrutor);
...
CREATE PROCEDURE stp_inserir_aula_pratica(param_quantidade ENUM('1', '2', '3'), param_id_instrutor INT, param_id_veiculo INT)
	BEGIN
		SET TRANSACTION ISOLATION LEVEL READ COMMITTED;
		INSERT INTO aula(tipo, codigo, quantidade, id_instrutor) VALUES('pratica', gera_codigo_aula(last_insert_id()), param_quantidade, param_id_instrutor);
...
CREATE PROCEDURE stp_inserir_aluno_aula(param_id_aluno INT, param_codigo_aula VARCHAR(10), param_data DATE, param_inicio TIME, param_fim TIME)
	BEGIN
		SET TRANSACTION ISOLATION LEVEL READ COMMITTED;
        INSERT INTO aluno_aula(id_aluno, id_aula, `data`, inicio, fim) VALUES(param_id_aluno, retorna_id_codigo_aula(param_codigo_aula), param_data, param_inicio, param_fim);
        COMMIT;
	END $$
