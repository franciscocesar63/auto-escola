...
CREATE PROCEDURE stp_inserir_pessoa(param_nome VARCHAR(80), param_sexo ENUM('masculino', 'feminino'), param_cpf VARCHAR(11), param_rg VARCHAR(11), 
	...
	BEGIN
        ...
        UPDATE pessoa SET pessoa.id_endereco = retorna_id_cpf(param_cpf) WHERE pessoa.id = retorna_id_cpf(param_cpf);
...
CREATE PROCEDURE stp_deleta_aluno(param_matricula VARCHAR(10))
	BEGIN
		...
		CALL stp_deleta_pessoa((SELECT pessoa.cpf FROM pessoa WHERE pessoa.id = retrona_id_matricula(param_matricula))); 
        CALL stp_deleta_aluno_aula((SELECT aluno_aula.id FROM aluno_aula WHERE aluno_aula.id_aluno = retrona_id_matricula(param_matricula)));
    END $$
...
CREATE PROCEDURE stp_deleta_funcionario(param_cpf VARCHAR(11))
	BEGIN
		CALL stp_deleta_pessoa(param_cpf);
        UPDATE funcionario SET funcionario.excluido = 1 WHERE funcionario.id = retorna_id(param_cpf);
		IF EXISTS(SELECT 1 FROM funcionario WHERE funcionario.cargo = 'instrutor' AND funcionario.id = retorna_id(param_cpf)) THEN
			UPDATE instrutor SET instrutor.excluido = 1 WHERE instrutor.id = retorna_id(param_cpf);
			UPDATE aula SET aula.excluido = 1 WHERE aula.id_instrutor = retorna_id(param_cpf);
		END IF; 
    END $$
