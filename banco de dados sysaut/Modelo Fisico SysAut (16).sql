...
CREATE TRIGGER trg_pessoa_verifica_excluido BEFORE UPDATE ON pessoa FOR EACH ROW
	BEGIN
		...
		UPDATE aluno INNER JOIN pessoa SET aluno.excluido = pessoa.excluido WHERE aluno.id = pessoa.id;
		UPDATE endereco INNER JOIN pessoa SET endereco.excluido = pessoa.excluido WHERE endereco.id = pessoa.id_endereco;
		UPDATE funcionario INNER JOIN pessoa SET funcionario.excluido = pessoa.excluido WHERE funcionario.id = pessoa.id; 
		UPDATE instrutor INNER JOIN pessoa SET  instrutor.excluido = pessoa.excluido WHERE instrutor.id = pessoa.id;
		UPDATE telefone INNER JOIN pessoa SET telefone.excluido = pessoa.excluido WHERE telefone.id_pessoa = pessoa.id;
		UPDATE usuario INNER JOIN pessoa SET usuario.excluido = pessoa.excluido WHERE usuario.id = pessoa.id;
...
CREATE FUNCTION retorna_id_cpf(param_cpf VARCHAR(11)) RETURNS INT  
	BEGIN
		DECLARE id INT;
		SELECT pessoa.id INTO id FROM pessoa WHERE pessoa.cpf = param_cpf;
        RETURN id;
    END $$
...
CREATE FUNCTION retorna_id_cep(param_cep VARCHAR(9)) RETURNS INT  
	BEGIN
		DECLARE id INT;
		SELECT endereco.id INTO id FROM endereco WHERE endereco.cep = param_cep;
		RETURN id;
    END $$
...
CREATE FUNCTION retorna_id_matricula(param_matricula VARCHAR(10)) RETURNS INT  
	BEGIN
		DECLARE id INT;
		SELECT aluno.id INTO id FROM aluno WHERE aluno.matricula = param_matricula;
        RETURN id;
    END $$
...
CREATE PROCEDURE stp_inserir_pessoa(param_nome VARCHAR(80), param_sexo ENUM('masculino', 'feminino'), param_cpf VARCHAR(11), param_rg VARCHAR(11), 
					param_email VARCHAR(80), param_numero_telefone VARCHAR(18), param_id_endereco INT)
	BEGIN
		INSERT INTO pessoa(nome, sexo, cpf, rg, email, telefone, id_endereco) VALUES(param_nome, param_sexo, param_cpf, param_rg, param_email, param_telefone, param_id_endereco);
...
CREATE PROCEDURE stp_atualiza_pessoa(param_id INT, param_nome VARCHAR(80), param_sexo ENUM('masculino', 'feminino'), param_email VARCHAR(80), 
					param_numero_telefone VARCHAR(18))
	BEGIN
...
CREATE PROCEDURE stp_inserir_endereco(param_logradouro VARCHAR(100), param_numero_casa VARCHAR(10), param_bairro VARCHAR(30), 
...
CREATE PROCEDURE stp_atuliza_endereco(param_logradouro VARCHAR(100), param_numero_casa VARCHAR(10), param_bairro VARCHAR(30), 
                    param_complemento VARCHAR(100), param_cidade VARCHAR(100), param_cep VARCHAR(9))
	BEGIN
...
CREATE PROCEDURE stp_inserir_aluno(param_id INT)
	BEGIN 
		INSERT INTO aluno(id, matricula, data_ingresso) VALUES(param_id, gera_matricula(year(now()), param_id), utc_date());
	END $$

CREATE PROCEDURE stp_inserir_funcionario(param_id INT, param_cargo ENUM('gerente', 'instrutor', 'secretaria'), param_salario DECIMAL)
		BEGIN
			INSERT INTO funcionario(id, cargo, salario, data_admissao) VALUES(param_id, param_cargo, param_salario, utc_date());
        END $$

CREATE PROCEDURE stp_inserir_usuario(param_id INT, param_login VARCHAR(30), param_senha VARCHAR(255))
	BEGIN
		INSERT INTO usuario(id, login, senha) VALUES(param_id, param_login, param_senha);
	END $$

CREATE PROCEDURE stp_inserir_instrutor(param_id INT, param_numero_cnh VARCHAR(30))
	BEGIN
        INSERT INTO instrutor(id, numero_cnh) VALUES (param_id, param_numero_cnh); 
    END $$
